...
import { DayOfWeek, DAYS_OF_WEEK, DAY_FULL_LABELS, HistoricalSalesData, EventData } from '@/lib/types';
...
export default function ForecastDataPage() {
  const { forecastDraft, setForecastDraft, saveForecast, discardForecast } = useAppState();
  const draft = forecastDraft.draft;
  const isDirty = forecastDraft.isDirty;
  ...
  const addSalesEntry = () => {
    ...
    setForecastDraft(prev => ({ ...prev, historicalSales: [...prev.historicalSales, entry] }));
  };

  const removeSalesEntry = (idx: number) => {
    setForecastDraft(prev => ({ ...prev, historicalSales: prev.historicalSales.filter((_, i) => i !== idx) }));
  };
  ...
  const addEventEntry = () => {
    ...
    setForecastDraft(prev => ({ ...prev, events: [...prev.events, entry] }));
  };

  const removeEventEntry = (idx: number) => {
    setForecastDraft(prev => ({ ...prev, events: prev.events.filter((_, i) => i !== idx) }));
  };
  ...
  const resetToDefaults = () => {
    setForecastDraft(getDefaultForecastInputs());
  };
    ...
    <div className="space-y-6 animate-fade-in max-w-3xl pb-20">
      ...
      <div className="stat-card space-y-4">
        <div className="flex items-center gap-2">
          ...
          <span className="ml-auto text-xs text-muted-foreground">{draft.historicalSales.length} entries</span>
        </div>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 items-end">
          <div>
            ...
            <Select value={salesDay} onValueChange={v => setSalesDay(v as DayOfWeek)}>
              ...
              <SelectContent>
                {DAYS_OF_WEEK.map(d => <SelectItem key={d} value={d}>{DAY_FULL_LABELS[d]}</SelectItem>)}
              </SelectContent>
            </Select>
          </div>
          <div>
            <Label className="text-xs">Hour (0-23)</Label>
            <Input type="number" min={0} max={23} value={salesHour} onChange={e => setSalesHour(+e.target.value)} className="mt-1" />
          </div>
          <div>
            <Label className="text-xs">Revenue ($)</Label>
            <Input type="number" min={0} value={salesRevenue} onChange={e => setSalesRevenue(+e.target.value)} className="mt-1" />
          </div>
          <Button size="sm" onClick={addSalesEntry} className="gap-1"><Plus className="w-3.5 h-3.5" /> Add</Button>
        </div>
              ...
              <tbody>
                {draft.historicalSales.map((s, i) => (
                  <tr key={i} className="border-t border-border">
                    ...
                    <td className="px-1">
                      <button onClick={() => removeSalesEntry(i)} className="p-1 text-muted-foreground hover:text-destructive transition-colors">
      ...
      <div className="stat-card space-y-4">
        <div className="flex items-center gap-2">
          ...
          <span className="ml-auto text-xs text-muted-foreground">{draft.events.length} events</span>
        </div>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 items-end">
          <div>
            <Label className="text-xs">Day</Label>
          ...
          <div>
            ...
            <Input type="number" min={0} value={eventAttendance} onChange={e => setEventAttendance(+e.target.value)} className="mt-1" />
          </div>
          <Button size="sm" onClick={addEventEntry} className="gap-1"><Plus className="w-3.5 h-3.5" /> Add</Button>
        </div>
              ...
              <tbody>
                {draft.events.map((e, i) => (
                  <tr key={i} className="border-t border-border">
                    ...
                    <td className="px-1">
                      <button onClick={() => removeEventEntry(i)} className="p-1 text-muted-foreground hover:text-destructive transition-colors">
                        <Trash2 className="w-3.5 h-3.5" />
                      </button>
                    </td>
                  </tr>
            ...
            return (
              <div key={day} className="grid grid-cols-3 gap-3 items-center py-1.5 px-2 rounded-md hover:bg-muted/50 transition-colors">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Input type="number" min={-20} max={50} value={w.temperature} onChange={e => updateWeather(day, 'temperature', +e.target.value)} className="h-8 text-xs" />
                </div>
                <div className="flex items-center gap-2">
                  <Label className="text-[10px] text-muted-foreground w-12 shrink-0">Rain %</Label>
                  <Input type="number" min={0} max={100} value={w.rainProbability} onChange={e => updateWeather(day, 'rainProbability', +e.target.value)} className="h-8 text-xs" />
                </div>
              </div>
            );
          })}
      ...
      <UnsavedChangesBar isDirty={isDirty} onSave={saveForecast} onDiscard={discardForecast} />
    </div>
  );
}
